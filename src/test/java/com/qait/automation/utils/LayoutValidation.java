	...
	public boolean checklayout(List<String> tagsToBeTested) {
		...
		if (getProperty("test-layout").equalsIgnoreCase("yes")) {
			try {
				...
				// this.filepath + this.platform + this.tier + this.PageName +
				// ".spec",
				// tagsToBeTested, null, null, null);

				LayoutReport layoutReport = Galen.checkLayout(this.driver,
						this.filepath + "WESTCOAST/" + this.PageName + ".spec", tagsToBeTested, null, null, null);
				...
				if (layoutReport.errors() > 0) {
					Reporter.log("\n[LAYOUT ERRORS]: There are Layout Errors on the page:- " + this.PageName
							+ "!!! The Errors are for ", true);
				...
				Reporter.log("[INFO]: Check reports in the location:- " + System.getProperty("user.dir")
						+ "\\target\\galen-reports" + "\n", true);
